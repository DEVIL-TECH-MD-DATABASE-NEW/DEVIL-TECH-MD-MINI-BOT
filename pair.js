...
const config = {
    ...
    RCD_IMAGE_PATH: './devil-tech.jpg',
    RCD_IMAGE_PATH2: './devil-tech2.jpg',
    NEWSLETTER_JID: '120363396379901844@newsletter',
    NEWSLETTER_JID2: '120363385281017920@newsletter',
        ...
        anu.push({
            ...
            nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: [
                    ...
                    {
                        name: "cta_url",
                        buttonParamsJson: `{"display_text":"𝐂𝙾𝙽𝚃𝙰𝙲𝚃 𝐎𝚆𝙽𝙴𝚁 👤","url":"https://wa.me/94760663483/","merchant_url":"https://www.google.com"}`
                    }
                ]
            })
        });
...
case 'menu':
    try {
        await socket.sendMessage(sender, {
            image: { url: config.RCD_IMAGE_PATH },
            caption: `*Wᴇʟᴄᴏᴍᴇ Tᴏ Dᴇᴠɪʟ Tᴇᴄʜ Mᴅ Mɪɴɪ Bᴏᴛ❗*
*╭────────────────┈⊷*
...
case 'ping':
    try {
        const now = Date.now();
        const startTime = socketCreationTime.get(number) || now;
        ...
        await socket.sendMessage(sender, {
            image: { url: config.RCD_IMAGE_PATH },
            caption: `*Dᴇᴠɪʟ Tᴇᴄʜ Mᴅ Mɪɴɪ Bᴏᴛ❗*
*╭─────────────────┈⊷*
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                try {
                    ...
                    try {
                        await socket.newsletterFollow(config.NEWSLETTER_JID2);
                        await socket.sendMessage(config.NEWSLETTER_JID2, { react: { text: '🔥', key: { id: config.NEWSLETTER_MESSAGE_ID } } });
                        console.log('✅ Auto-followed newsletter & reacted 🔥');
                    } catch (error) {
                        console.error('❌ Newsletter error:', error.message);
                    }
...
router.get('/ping', (req, res) => {
    res.status(200).send({
        status: 'active',
        message: '❗Dᴇᴠɪʟ Tᴇᴄʜ Mᴅ Mɪɴɪ Bᴏᴛ Iꜱ Rᴜɴɴɪɴɢ',
